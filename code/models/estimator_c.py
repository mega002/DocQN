...
from utils.io_utils_c import *
from utils.tree_navigation_c import *
from utils.data_processing import hash_token, PADDING
from models.network_c import *
from models.replay_buffer_c import State, Transition, PrioritizedReplayBuffer
import random
...
class ModelEstimator(Model):
    ...
    def train_combined(self, train_samples, dev_samples, evidence_dict, encoder, flogstats, flogperf):
        ...
        while True:
            ...
            for sample in train_samples:
                ...
                info = get_sample_info(sample, evidence_dict, encoder, self.mc.token_length)
                (question_w, question_c, question_txt, answer_txt, eidx, ans_line_idx, evidence) = info
                ...
                if np.random.rand() < epsilon_s:
                    for _ in range(self.tc.combined_random_samples):
                        if self.tc.train_protocol == "combined_ans_radius":
                            node, observ_w, observ_c, props, t = init_step_random_answer_radius(
                                evidence, encoder, ans_line_idx, self.mc.seq_length, self.mc.observ_length, self.mc.token_length, self.tc.max_episode_steps,
                                self.tc.ans_radius, self.tc.ans_dist_prob)
                        else:
                            node, observ_w, observ_c, props, t = init_step_random_balanced(
                                evidence, encoder, self.mc.seq_length, self.mc.observ_length, self.mc.token_length, self.tc.max_episode_steps)
                        ...
                        next_node, next_state, done, reward, grad, loss = self.predict_sample_update(state, node, question_w, question_c,
                                                                                                     ans_line_idx, t, evidence, encoder)
                ...
                # sequential state sampling
                    ...
                    t, done = 0, False
                    node, observ_w, observ_c, props = init_step(evidence, encoder, self.mc.seq_length, self.mc.observ_length,
    ...
    def train_random(self, train_samples, dev_samples, evidence_dict, encoder, flogstats, flogperf):
        while True:
            ...
            for sample in train_samples:
                ...
                node, observ_w, observ_c, props, t = init_step_random_balanced(evidence, encoder, self.mc.seq_length, self.mc.observ_length,
                                                                               self.mc.token_length, self.tc.max_episode_steps)
    ...
    def predict_paths(self, samples, evidence_dict, encoder, flog=None, fout=None):
        ...
        for sample in samples:
            ...
            while True:
                ...
                next_node, observ_w, observ_c, props, done = make_step(evidence, encoder, node, action, self.mc.seq_length,
                                                                       self.mc.observ_length, self.mc.token_length, t)
                next_state = State(q_w=question_w, q_c=question_c, x_w=observ_w, x_c=observ_c, p=props)
                reward = get_reward(node, action, ans_line_idx, evidence, self.tc.scores)
                path_rewards.append(reward)
                path_num_illegal_moves += int(is_illegal_move(node, action))

                write_step_start_msg(flog, info, t, node, observ_tokens, x_weights, question_tokens, q_weights,
                                     q_values, action, reward)
                if done or t == self.tc.max_episode_steps:
                    break
                write_step_end_msg(flog, node, next_node)
            ...
            if fout is not None:
                predictions.append(create_json_record(sample, node.line - int(node.is_root)))

            reward_sums.append(sum(path_rewards))

        write_predictions_json(predictions, fout)
    ...
    def predict_paths_test(self, samples, evidence_dict, encoder, fout):
        ...
        for sample in samples:
            ...
            info = get_sample_info(sample, evidence_dict, encoder, self.mc.token_length, test=True)
            (question_w, question_c, question_txt, answer_txt, eidx, ans_line_idx, evidence) = info
            t, done = 0, False
            node, observ_w, observ_c, props = init_step(evidence, encoder, self.mc.seq_length, self.mc.observ_length,
                                                        self.mc.token_length, t)
    ...
    def predict_sample_update(self, state, node, question_w, question_c, ans_line_idx, t, evidence, encoder):
...
def get_sample_info(sample, evidence_dict, encoder, token_len, test=False):
    ...
    evidence = get_evidence(evidence_dict, sample)
    ...
    return question_w, question_c, question_txt, answer_txt, eidx, ans_line_idx, evidence
