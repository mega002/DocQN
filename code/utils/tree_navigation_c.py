...
def get_evidence(evidence_dict, sample):
    ...
    if isinstance(evidence_dict, EvidenceRpcClient):
        return evidence_dict.call('<EMPTY>--' + evidence_title)
    else:
        return evidence_dict[evidence_title]


def init_step(evidence, encoder, seq_len, observ_len, token_len, step):
    root = evidence["tree"]
    observ_w, observ_c, props = get_node_observ_props(root, step, evidence, encoder, seq_len, observ_len, token_len)

    return root, observ_w, observ_c, props


def init_step_random_balanced(evidence, encoder, seq_len, observ_len, token_len, max_episode_steps):
    ...
    observ_w, observ_c, props = get_node_observ_props(node, step, evidence, encoder, seq_len, observ_len, token_len)

    return node, observ_w, observ_c, props, step


def init_step_random_answer_radius(evidence, encoder, ans_line_idx, seq_len, observ_len, token_len, max_episode_steps,
                                   ans_radius, ans_dist_prob):
    if np.random.rand() > ans_dist_prob:
        return init_step_random_balanced(evidence, encoder, seq_len, observ_len, token_len, max_episode_steps)
...
def get_random_nearby_node(start_node, max_actions, evidence):
    ...
    for _ in range(num_actions):
        ...
        if len(legal_actions) == 0:
            print("unexpected behavior - only STOP is a leagal action:\n\nstart_node: {}\t\nnode: {}".format(
                start_node, node))
            return node
...
def make_step(evidence, encoder, node, action, seq_len, observ_len, token_len, step):
    ...
    # STOP
        ...
        return node, observ_w, observ_c, props, done

    observ_w, observ_c, props = get_node_observ_props(new_node, step, evidence, encoder, seq_len, observ_len, token_len)
    done = False

    return new_node, observ_w, observ_c, props, done
...
def get_node_observ_props(node, step, evidence, encoder, seq_len, observ_len, token_len):
    ...
    return observ_w, observ_c, props
